...
{
    [Trait("Category", "ClientLibraryTests")]
    public class KclTest
    {
        ...
        public void HappyCase()
        {
            ...
            Assert.True(a is StatusAction);
            Assert.True(a.ResponseFor == "initialize");


            a = outputActions[1];
            Assert.True(a is CheckpointAction);
            Assert.True(a.SequenceNumber == "456" && a.Error == null);


            a = outputActions[2];
            Assert.True(a is StatusAction);
            Assert.True(a.ResponseFor == "processRecords");


            a = outputActions[3];
            Assert.True(a is CheckpointAction);
            Assert.True(a.SequenceNumber == null && a.Error == null);


            a = outputActions[4];
            Assert.True(a is StatusAction);
            Assert.True(a.ResponseFor == "shutdown");
        }
        ...
        public void RetryHandler()
        {
            ...
            Assert.True(a is StatusAction, "Action " + (i - 1) + " should be StatusAction");
            Assert.Equal("initialize", a.ResponseFor);

            for (int j = 0; j <= numRetries; j++)
            {
                a = outputActions[i++];
                Assert.True(a is CheckpointAction, "Action " + (i - 1) + " should be CheckpointAction");
                Assert.Equal("456", a.SequenceNumber);
                Assert.Null(a.Error);
            }

            a = outputActions[i++];
            Assert.True(a is StatusAction, "Action " + (i - 1) + " should be StatusAction");
            Assert.Equal("processRecords", a.ResponseFor);

            for (int j = 0; j <= numRetries; j++)
            {
                a = outputActions[i++];
                Assert.True(a is CheckpointAction, "Action " + (i - 1) + " should be CheckpointAction");
                Assert.Null(a.SequenceNumber);
                Assert.Null(a.Error);
            }

            a = outputActions[i++];
            Assert.True(a is StatusAction, "Action " + (i - 1) + " should be StatusAction");
            Assert.Equal("shutdown", a.ResponseFor);
